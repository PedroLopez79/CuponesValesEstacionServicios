...
begin
  inherited;
  if (Key = 8) or (Key = 46) and (lblCupon.Caption <> '') then
     lblCupon.Caption:= Copy(lblCupon.Caption, 0, length(lblCupon.Caption)-1)
  else
     lblCupon.Caption:= lblCupon.Caption + KEYTOSTRING(Key);
...
function TfrmCuponInterno.KEYTOSTRING(KEY: WORD): String;
var
...
initialization
  ModuleInfoManager.RegisterModule('Cupón Interno', TfrmCuponInterno);
