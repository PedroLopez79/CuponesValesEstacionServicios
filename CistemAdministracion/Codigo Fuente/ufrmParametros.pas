...
uses
  ...
  LibraryValesCupones_Intf, cxClasses, dxRibbon, DB, uDAInterfaces,
...
procedure TfrmParametros.cxClienteFinExit(Sender: TObject);
begin
  cdsClientes.Close;
  cdsClientes.ParamByName('ClienteID').AsInteger:=cxClienteFin.EditValue;
  cdsClientes.Open;
end;

procedure TfrmParametros.cxClienteIniExit(Sender: TObject);
begin
  cdsCliente.Close;
  cdsCliente.ParamByName('ClienteID').AsInteger:=cxClienteIni.EditValue;
  cdsCliente.Open;
end;
...
begin
  ...
  cxTextClienteIni.Text:= cdsCliente.ParamByName('Nombre').AsString;
end;
